...
/*        	 String[] time = df.format(System.currentTimeMillis()).split("-");
     ...
     public static  List<HashMap<String, Object>> getDiskMapList() throws SigarException{
         Sigar sigar = new Sigar();
         FileSystem fslist[] = sigar.getFileSystemList();
         ...
         try {
               for (int i = 0; i < fslist.length; i++) {
                 ...
                 case 2: // TYPE_LOCAL_DISK : 本地硬盘
                     // 文件系统总大小
                     map.put("totalSize", usage.getTotal());
                     // 文件系统剩余大小
                     map.put("freeSize", usage.getFree());
                     // 文件系统可用大小
                     map.put("availableSize", usage.getAvail());
                     // 文件系统已经使用量
                     map.put("usedSize", usage.getUsed());
                     // 文件系统资源的利用率
                     map.put("usePerc", usage.getUsePercent() * 100D);
                     break;
                 case 3:// TYPE_NETWORK ：网络
                     break;
                 case 4:// TYPE_RAM_DISK ：闪存
                 ...
                 map.put("diskRead", usage.getDiskReads());
                 map.put("diskWrite", usage.getDiskWrites());
